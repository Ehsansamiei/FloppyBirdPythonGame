...
game_status = True
bird_list_index = 0
game_font = pygame.font.Font(r'.\asset\font\Flappy.TTF', 40)
score = 0
high_score = 0
active_score = True
...
# ---------- #
win_sound = pygame.mixer.Sound(r'.\asset\sound\smb_stomp.wav')
game_over_sound = pygame.mixer.Sound(r'.\asset\sound\smb_mariodie.wav')
# ---------- #
background_image = pygame.transform.scale2x(
    pygame.image.load(r'.\asset\img\bg2.png'))
floor_image = pygame.transform.scale2x(
    pygame.image.load(r'.\asset\img\floor.png'))
bird_image_down = pygame.transform.scale2x(
    pygame.image.load(r'.\asset\img\red_bird_down_flap.png'))
bird_image_mid = pygame.transform.scale2x(
    pygame.image.load(r'.\asset\img\red_bird_mid_flap.png'))
bird_image_up = pygame.transform.scale2x(
    pygame.image.load(r'.\asset\img\red_bird_up_flap.png'))
...
pipe_image = pygame.transform.scale2x(
    pygame.image.load(r'.\asset\img\pipe_red.png'))
game_over_image = pygame.transform.scale2x(
    pygame.image.load(r'.\asset\img\message.png'))
game_over_image_rect = game_over_image.get_rect(center=(288, 512))


def generate_pipe_rect():
    random_pipe = random.randrange(400, 550)
    pipe_rect_top = pipe_image.get_rect(midbottom=(700, random_pipe - 250))
...
def check_collision(pipes):
    ...
    for pipe in pipes:
        if bird_image_rect.colliderect(pipe):
            ...
            active_score = True
            return False
        if bird_image_rect.top <= -50 or bird_image_rect.bottom >= 600:
            game_over_sound.play()
            time.sleep(3)
            active_score = True
            return False
...
while True:
    ...
    main_screen.blit(floor_image, (floor_x + 576, 600))
    if floor_x <= -576:
        floor_x = 0
